...
| 種別 | 仕様 |
|------|------|
| ホーム | 全ユーザーの公開投稿。新着順（要認証） |
| 探索（全体） | 全ユーザーの公開投稿。新着順（未認証でも閲覧可） |
...
| 画面名 | パス | 説明 |
|-------|------|------|
| ホーム | `/` | 全ユーザーの投稿タイムライン（要ログイン） |
| 探索 | `/explore` | 全ユーザーの投稿タイムライン・検索 |
...
### 投稿
```
GET    /api/v1/posts              (探索: 全体タイムライン、未認証可)
GET    /api/v1/posts/home         (ホーム: 全体タイムライン、要認証)
POST   /api/v1/posts
GET    /api/v1/posts/:id
PUT    /api/v1/posts/:id
